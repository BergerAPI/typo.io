<script>
    ...
    function updateCaret(animation = true) {
        ...
        const currentLetter = document.getElementById(isLastLetter ? text.length - 1 : writtenText.length.toString())
        ...
        if(isLastLetter) left += boundingRect.width
...
<svelte:window on:keydown={handleKeydown} on:resize={() => updateCaret(false)} on:load={() => updateCaret(false)}/>
